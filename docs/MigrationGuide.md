...
Аргументы методов `select`, `excludeSelect`, `with`, `where` и `orderBy` у `SearchQuery` получили типизацию. Runtime-поведение не менялось: старые строковые пути, alias вида `model.timeFrom` и вручную собранные условия продолжают поддерживаться.

Чтобы получить подсказки, `SearchQuery` должен знать тип модели. Обычно это происходит автоматически при вызове `createQuery()` из типизированного `CrudService` или `ReadService`. Если `SearchQuery` создаётся вручную, передайте тип модели в generic:
...
- Если в проекте используется `orderBy('model.timeFrom')`, замените его на `orderBy('timeFrom')`, чтобы получить строгие подсказки типов. Менять существующий вызов не обязательно: вариант с корневым alias остаётся валидным для обратной совместимости.
- Если в `with` используется объектное поле без `id`, оно не будет предложено как путь связи. Для типизированного `with` используйте поле связи — объект или массив объектов с полем `id`, либо поле с суффиксом `Id`/`Ids`.
- Если используется dot-path глубже пяти сегментов, передавайте его строкой вручную. Такой путь продолжает работать, но не входит в строгие подсказки.
- Если условия собираются через `map` и spread, TypeScript может вывести обычный массив вместо tuple-условия. В этом случае укажите явный тип результата:
